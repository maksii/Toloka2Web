...
def search_aggregated():
    try:
        ...
        error_message = {"error": "Failed to perform search", "details": str(e)}
...
def proxy_image():
    try:
        ...
        error_message = {"error": "Failed to proxy image", "details": str(e)}
        return make_response(jsonify(error_message), 500)
...
def configure_routes(app, login_manager, admin_permission, user_permission):
    ...
    def before_request():
        if current_user.is_authenticated:
            identity_changed.send(
                current_app._get_current_object(), identity=Identity(current_user.id)
            )
