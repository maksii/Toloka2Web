...
success_response = api.model(
    "SuccessResponse", {"message": fields.String(description="Success message")}
)
...
search_input = api.model(
    "SearchInput", {"query": fields.String(required=True, description="Search query")}
)
...
release_model = api.model(
    "Release",
    {
        "hash": fields.String(description="Release hash"),
        "title": fields.String(description="Release title"),
        "torrent_info": fields.Nested(
            torrent_info_model, description="Torrent information"
        ),
...
releases_list_response = api.model(
...
release_input = api.model(
    "ReleaseInput",
    {
        "title": fields.String(required=True, description="Release title"),
        "hash": fields.String(required=True, description="Release hash"),
        "status": fields.String(description="Release status"),
        "release_group": fields.String(
            description="Release group (optional, e.g. when adding release)"
        ),
        "meta": fields.String(
            description="Meta/tags (optional, e.g. WEBDL; default from Toloka default_meta)"
        ),
    },
)

release_defaults_response = api.model(
    "ReleaseDefaultsResponse",
    {
        "default_meta": fields.String(
            description="Default meta value from Toloka section in app.ini (synced to DB)"
        )
    },
)
...
anime_list_response = api.model(
    "AnimeListResponse", {"data": fields.List(fields.Nested(anime_model))}
)

studio_list_response = api.model(
    "StudioListResponse", {"data": fields.List(fields.Nested(studio_model))}
)

user_list_response = api.model(
    "UserListResponse", {"data": fields.List(fields.Nested(user_info))}
)

settings_list_response = api.model(
    "SettingsListResponse", {"data": fields.List(fields.Nested(setting_model))}
)

mal_search_response = api.model(
    "MALSearchResponse",
    {"data": fields.List(fields.Raw(description="MAL search results"))},
)

tmdb_search_response = api.model(
    "TMDBSearchResponse",
    {"data": fields.List(fields.Raw(description="TMDB search results"))},
)

# Stream Models
stream_search_input = api.model(
    "StreamSearchInput",
    {"query": fields.String(required=True, description="Search query")},
)

stream_add_input = api.model(
    "StreamAddInput",
    {
...
stream_details_input = api.model(
    "StreamDetailsInput",
    {
        "provider": fields.String(required=True, description="Streaming provider"),
        "link": fields.String(required=True, description="Title link"),
...
# Toloka Models
toloka_torrent_model = api.model(
    "TolokaTorrent",
    {
...
toloka_search_response = api.model(
    "TolokaSearchResponse",
    {
...
aggregated_search_response = api.model(
...
# Auth Check Models
auth_check_response = api.model(
    "AuthCheckResponse",
    {
        "authenticated": fields.Boolean(description="Authentication status"),
        "user": fields.Nested(
            user_info, description="User information if authenticated"
        ),
    },
)

# Image Proxy Models
image_proxy_response = api.model(
    "ImageProxyResponse",
    {
