        ...
        for migration in migrations:
            ...
            if migration["column"] not in columns:
                try:
                    ...
                    conn.commit()
                    app.logger.info(
                        f"Migration: Added '{migration['column']}' column to '{migration['table']}' table"
                    )
                except sqlite3.Error as e:
                    app.logger.warning(
                        f"Migration warning for {migration['column']}: {e}"
                    )
...
def create_app(test_config=None):
    ...
    def token_not_fresh_callback(jwt_header, jwt_payload):
        return jsonify(
            {"error": "Fresh token required", "code": "fresh_token_required"}
        ), 401
